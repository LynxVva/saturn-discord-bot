...
class ErrorHandler(commands.Cog):
    ...
    async def raise_exception(self, exc, ctx: commands.Context) -> None:
        ...
        send_embed = SaturnEmbed(
            description=f"{CROSS} Something went wrong. Whoops! "
    ...
    async def on_command_error(self, ctx, exc):
        ...
        if isinstance(exc, commands.CommandNotFound):
            ...
            em = SaturnEmbed(
                description=f"{CROSS} Whoops, something didn't go as intended. Check my permissions and try again.",
                colour=RED)
            return await ctx.send(embed=em)

        elif isinstance(exc, discord.Forbidden):
            em = SaturnEmbed(
                description=f"{CROSS} I do not have permission to do that! Check my role's permissions, and try again.",
                colour=RED)
            return await ctx.send(embed=em)
            ...
            em = Embed(
                description=f"{CROSS} Invalid argument `{parameter}` passed\n"
            ...
            em = Embed(
                description=f"{CROSS} Woah there, too fast! "
            ...
            em = Embed(
                description=f"{CROSS} No such channel was found.",
                colour=RED)
            await ctx.send(embed=em)
            ...
            em = Embed(
                description=f"{CROSS} You do not have  the proper permissions perform this action.\n"
            ...
            perms = str(', '.join(exc.missing_perms)).title().replace('_', ' ')
            em = Embed(
                description=f"{CROSS} I do not have the proper permissions to perform this action.\n"
                            f"```Missing {perms} permissions```",
                colour=RED)
            ...
            em = Embed(
                description=f"{CROSS} Invalid argument passed\n"
            ...
            em = Embed(
                description=f"{CROSS} You are not a developer of this bot.",
                colour=RED)
            await ctx.send(embed=em)

        elif isinstance(exc, commands.CheckFailure):
            em = Embed(
                description=f"{CROSS} You do not have the proper permissions to perform this action.",
                colour=RED)
            await ctx.send(embed=em)

        elif isinstance(exc, commands.DisabledCommand):
            em = Embed(
                description=f"{CROSS} This command is currently disabled.",
                colour=RED)
            await ctx.send(embed=em)

        elif isinstance(exc, commands.TooManyArguments):
            em = Embed(
                description=f"{CROSS} Too many arguments were passed.",
                colour=RED)
            await ctx.send(embed=em)
